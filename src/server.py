...
from _server_helper import (
    extract_round_client_id_payload,
    response_if_not_responder,
    build_keyset_response,
    build_sharekeys_response,
    build_masked_input_response,
    build_consistency_check_response,
)
...
class SecureAggregationServer:
    ...
    def __init__(self):
        ...
        }  # stores per-round received data
        self.lock = threading.Lock()  # locks state while edits being made
        self.app = Flask(
            __name__
        )  # Flask app instance to receive/send HTTP/POST requests (calls _setup_routes)
        self.roundi_responders = {
            r: set() for r in range(1, ROUNDS + 1)
        }  # Clients that responded in each round
        self.roundi_threshold_met = {
            r: threading.Event() for r in range(1, ROUNDS + 1)
        }  # Event to signal when threshold is reached
        self.roundi_responders_locked = {
            r: threading.Event() for r in range(1, ROUNDS + 1)
        }  # Event to signal when threshold wait period ENDS
        self.roundi_result = {
            r: None for r in range(1, ROUNDS + 1)
        }  # Computed result after each round's threshold wait
    ...
    def _setup_routes(self):
        ...
        self.app.route("/round5/result", methods=["GET"])(self.get_round5_result)
    ...
    def handle_round(self, round_num: int):
        ...
        (round, client_id, payload) = extract_round_client_id_payload(
            data, expected_round=round_num
        )
        ...
        with self.lock:
            ...
            print(
                f"Received from client {client_id} in round {round}: {payload}",
                flush=True,
            )
            ...
            if round == 1:
                ...
                pre_check_response = self._check_round5(client_id)
    ...
    def _check_round2(self, client_id: int) -> dict:
        ...
        if not self.roundi_responders_locked[1].is_set():
            return {
                "status": "error",
                "message": "Round 2 received before round 1 threshold wait completed.",
            }
    ...
    def _check_round3(self, client_id: int) -> dict:
        ...
        if not self.roundi_responders_locked[2].is_set():
            return {
                "status": "error",
                "message": "Round 3 received before round 2 threshold wait completed.",
            }
    ...
    def _check_round4(self, client_id: int) -> dict:
        ...
        if not self.roundi_responders_locked[3].is_set():
            return {
                "status": "error",
                "message": "Round 4 received before round 3 threshold wait completed.",
            }
        elif client_id not in self.roundi_responders[3]:
            return response_if_not_responder(client_id, 3)
        return None

    def _check_round5(self, client_id: int) -> dict:
        """Pre-check for round 5. Returns None if checks pass, or error response if they fail."""
        # Check if client participated in round 4
        if not self.roundi_responders_locked[4].is_set():
            return {
                "status": "error",
                "message": "Round 5 received before round 4 threshold wait completed.",
            }
        elif client_id not in self.roundi_responders[4]:
            return response_if_not_responder(client_id, 4)
        return None
    ...
    def _collect_round_response(self, round: int, client_id: int, payload) -> dict:
        ...
        max_responders = (
            MAX_CLIENTS if round == 1 else len(self.roundi_responders[round - 1])
        )

        # If we haven't hit the threshold yet, add this client to responders
        if (
            not self.roundi_responders_locked[round].is_set()
            and len(self.roundi_responders[round]) < max_responders
        ):
            self.roundi_responders[round].add(client_id)
            print(
                f"Round {round}: Client {client_id} added. Responders so far: {len(self.roundi_responders[round])} / Threshold: {THRESHOLD_CLIENTS}",
                flush=True,
            )
            ...
            if len(self.roundi_responders[round]) >= THRESHOLD_CLIENTS:
                print(
                    f"Round {round}: Threshold reached ({THRESHOLD_CLIENTS} clients). Starting wait period of {THRESHOLD_WAITS[round]} seconds.",
                    flush=True,
                )
                ...
                if not self.roundi_responders_locked[round].is_set():
                    threshold_wait_thread = threading.Thread(
                        target=self._threshold_wait, args=(round,)
                    )
            ...
            return {
                "status": "ok",
                "message": f"Client {client_id} registered. Waiting for at least {max(THRESHOLD_CLIENTS-len(self.roundi_responders[round]), 0)} more clients.",
            }
            ...
            return {
                "status": "ok",
                "message": f"Client {client_id} registered for round {round}.",
            }
    ...
    def _threshold_wait(self, round: int):
        ...
        print(
            f"Starting thread waiting for round {round} threshold wait period ({THRESHOLD_WAITS[round]}s)...",
            flush=True,
        )
        ...
        with self.lock:
            print(
                f"Round {round}: Wait period complete. Final responders: {responders}",
                flush=True,
            )
    ...
    def get_round1_result(self):
        ...
        with self.lock:
            if client_id in self.roundi_responders[1]:
                response = build_keyset_response(
                    self.received_data, self.roundi_responders[1]
                )
            else:
    ...
    def get_round2_result(self):
        ...
        with self.lock:
            if client_id not in self.roundi_responders[1]:
                ...
                response = build_sharekeys_response(
                    client_id, self.received_data, self.roundi_responders[2]
                )
        return jsonify(response)
    ...
    def get_round3_result(self):
        ...
        with self.lock:
            if client_id not in self.roundi_responders[1]:
                ...
                response = build_masked_input_response(
                    client_id, self.received_data, self.roundi_responders[3]
                )
        return jsonify(response)

    def get_round4_result(self):
        """Clients poll this endpoint to get the round 4 (ConsistencyCheck) result once threshold wait completes."""
        ...
        with self.lock:
            if client_id not in self.roundi_responders[3]:
                ...
                response = build_consistency_check_response(
                    self.received_data,
                    self.roundi_responders[3],
                    self.roundi_responders[4],
                )
        return jsonify(response)

    def get_round5_result(self):
        """Clients poll this endpoint to get the round 5 (Unmasking) result with the final aggregate."""
        print("Get round 5 result called", flush=True)
        # Extract client_id from query parameter
        client_id = request.args.get("client_id", type=int)

        # Wait until round 5 is complete and result is ready
        self.roundi_responders_locked[5].wait()

        with self.lock:
            if self.final_aggregate is None:
                some_y = next(iter(self.received_data[3].values()))
                vec_len = len(some_y)
                self.final_aggregate = compute_final_aggregate(
                    self.received_data,
                    self.roundi_responders[2],
                    self.roundi_responders[3],
                    self.received_data[5],
                    vec_len,
                )
            if client_id not in self.roundi_responders[4]:
                response = response_if_not_responder(client_id, 4)
            elif client_id not in self.roundi_responders[5]:
                response = response_if_not_responder(client_id, 5)
            else:
