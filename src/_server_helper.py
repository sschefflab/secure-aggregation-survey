...
def compute_final_aggregate(received_data: dict[int, dict[int, dict]], round2_responders: set[int], round3_responders: set[int], round4_data: set[int], vec_len: int) -> list:
	# Implementation for computing final aggregate
	...
	for u in round3_responders:
		...
		for i in range(vec_len):
			z[i] += field_add(z[i], y_u[i])

	for u in survived:
		prg_seed_shares = []
		for reported_id, reporter_shares in round4_data.items():
			target_key = str(u)
