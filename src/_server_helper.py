...
def extract_round_client_id_payload(
    data: dict, expected_round: int
) -> tuple[int, int, dict]:
    ...
    ), f"Round number {expected_round} from POST does not match JSON body round number {round}"
    ...
    if not (1 <= client_id <= MAX_CLIENTS):
        raise ValueError(
            f"Error: Invalid client_id {client_id}; must be 1..{MAX_CLIENTS}"
        )
...
def build_keyset_response(
    received_data: dict[int, dict[int, dict]], round1_responders: set[int]
) -> dict[int, dict]:
...
def build_sharekeys_response(
    client_id: int,
    received_data: dict[int, dict[int, dict]],
    round2_responders: set[int],
) -> dict[int, dict]:
    # received_data format: {round: {client_id: {payload}}}
    ...
    for other_id in round2_responders:
        ...
        if other_id not in round2_data or str(client_id) not in round2_data[other_id]:
            raise ValueError(
                f"Error: build_sharekeys_response called for client {client_id}, but no round 2 data from other client {other_id} for this client"
            )
        to_ret[other_id] = round2_data[other_id][
            str(client_id)
        ]  # add received nonce and ciphertext from other_id for client_id to client_id's dictionary
    return to_ret


def build_masked_input_response(
    client_id: int,
    received_data: dict[int, dict[int, dict]],
    round3_responders: set[int],
) -> dict:
    return {"u2_users": sorted(list(round3_responders))}


def build_consistency_check_response(
    received_data: dict, round3_responders: set[int], round4_responders: set[int]
) -> dict:
    u3_list = sorted(list(round3_responders))
    signatures = {}
    for uid in round4_responders:
        signatures[str(uid)] = received_data[4][uid]["signature"]
    return {
        "users": {str(uid): {} for uid in round4_responders},
        "u2_users": u3_list,
        "u2_signatures": signatures,
    }


def compute_final_aggregate(
    received_data, round2_responders, round3_responders, round5_data, vec_len
):
    """
    ...
    for u in survived:
        ...
        for reporter_id, reporter_shares in round5_data.items():
            target_key = str(u)
            if (
                target_key in reporter_shares
                and reporter_shares[target_key]["type"] == "survived"
            ):
    ...
    for d in dropped:
        ...
        for reporter_id, reporter_shares in round5_data.items():
            ...
            if (
                target_key in reporter_shares
                and reporter_shares[target_key]["type"] == "dropped"
            ):
...
def response_if_not_responder(client_id: int, round_failed_to_respond: int) -> dict:
    return {
        ...
        "message": f"Client {client_id} responded too late to participate in Round {round_failed_to_respond}.",
    }
