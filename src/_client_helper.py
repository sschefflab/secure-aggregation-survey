...
def _poll_for_round5_result(client_id: int, server_url: str) -> dict:
   ...
   for poll_count in range(MAX_POLLS[5]):
       try:
           result_resp = requests.get(f"{server_url}/round5/result?client_id={client_id}", timeout=5)
           if result_resp.status_code == 200:
               print(f"Client {client_id}: Got round 5 result after {poll_count} polls", flush=True)
               return result_resp.json()
       except requests.exceptions.Timeout:
           pass
       except requests.exceptions.RequestException as e:
