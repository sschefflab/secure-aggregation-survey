...
class SecureAggregationClient:
   ...
   def unmasking(self, r4_response: dict) -> dict:
       ...
       if len(round4_responders) < THRESHOLD_CLIENTS:
           print(f"Secutity Alert: Only {len(round4_responders)} users survived, aborting.", flush=True )
           sys.exit(1)
...
def main():
   ...
   parser.add_argument('--vec', type=str, default="1,2,3", help='Input vector as comma-separated integers, e.g. "1,2,3"')
   ...
   client = SecureAggregationClient(client_id=client_id, x_u=x_u, signingkeyfile=args.signingkey, verificationkeysfile=args.verificationkeys)
