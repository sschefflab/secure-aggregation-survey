...
class SecureAggregationClient:
        ...
        if signingkeyfile is not None and verificationkeysfile is not None:
            ...
            with open(verificationkeysfile, 'rb') as f:
                ...
                for client_id, pubkey_hex in verification_keys.items():
                    self.verificationkeys[int(client_id)] = ed25519.Ed25519PublicKey.from_public_bytes(bytes.fromhex(pubkey_hex))
    ...
    def share_keys(self, r1_response: dict) -> dict:
        ...
        for r1r in self.round1_responders:
            ...
            try:
                ...
                print(f"Client {self.client_id} failed to verify signature of client {r1r} in round 1, aborting. Error: {e}", flush=True)
                sys.exit(1)
    ...
    def consistency_check(self, r3_response: dict) -> dict:
        u2_list_from_server = r3_response.get('u2_users', [])
        if not u2_list_from_server or len(u2_list_from_server) < THRESHOLD_CLIENTS:
            print(f"Consistency check: insufficient participants, aborting", flush=True)
            sys.exit(1)
        
        u2_list_from_server.sort()
        message_to_sign = json.dumps(u2_list_from_server).encode('utf-8')
    ...
    def unmasking(self, r4_response: dict) -> dict:
        ...
        if len(self.round3_responders) < THRESHOLD_CLIENTS:
            print(f"Secutity Alert: Only {len(self.round3_responders)} users survived, aborting.", flush=True )
            sys.exit(1)
        
        #active adversary check: verify all signatures of the u2 list from round 3
        u2_list_from_server = r4_response.get('u2_users', [])
        message_to_verify = json.dumps(u2_list_from_server).encode('utf-8')
        for other_id, sig in r4_response['u2_signatures'].items():
            other_id_int = int(other_id)
            if other_id_int == self.client_id:
                continue
            try:
                self.verificationkeys[other_id_int].verify(bytes.fromhex(sig), message_to_verify)
            except Exception as e:
                print(f"Client {self.client_id} failed to verify signature of client {other_id} in round 4, aborting. Error: {e}", flush=True)
                sys.exit(1)
...
def main():
    ...
    client = SecureAggregationClient(client_id=client_id, x_u=x_u, signingkey=args.signingkey, verificationkeys=args.verificationkeys)
    ...
    r3_response = do_round(client_id, 3, client.masked_input_collection(r2_response), SERVER_URL)
    r4_response = do_round(client_id, 4, client.consistency_check(r3_response), SERVER_URL)
